...
object Nonblocking:
  ...
  object Par:
    ...
    def choiceN[A](p: Par[Int])(ps: List[Par[A]]): Par[A] = {
      val l = ps.length
      es => cb => p(es)(ind => eval(es)(ps(ind % l)(es)(cb)))
    }

    def choiceViaChoiceN[A](cond: Par[Boolean])(ifTrue: Par[A], ifFalse: Par[A]): Par[A] =
      choiceN(cond.map(b => if b then 0 else 1))(List(ifTrue, ifFalse))

    def choiceMap[K, V](p: Par[K])(ps: Map[K, Par[V]]): Par[V] =
      es => cb => p(es) { k => eval(es)(ps(k)(es)(cb)) }

    /* `chooser` is usually called `flatMap` or `bind`. */
    def chooser[A, B](p: Par[A])(f: A => Par[B]): Par[B] =
      es => cb => p(es) { a => eval(es)(f(a)(es)(cb)) }
